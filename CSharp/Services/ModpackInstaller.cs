...
namespace Launcher.Services
{
    ...
    public class ModpackInstaller : IModpackInstaller
    {
        ...
        public async Task<MinecraftInstance> InstallModpackAsync(string mrpackUrl, string modpackName, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            var bytes = await _httpClient.GetByteArrayAsync(mrpackUrl, cancellationToken);
            ...
            try
            {
                ...
                if (index.Dependencies != null)
                {
                    ...
                    else if (index.Dependencies.TryGetValue("neoforge", out var neoforgeVer)) { loader = "Forge"; loaderVersion = neoforgeVer; }
                }
                ...
                foreach (var file in files)
                {
                    ...
                    var fileBytes = await _httpClient.GetByteArrayAsync(url, cancellationToken);
                    await File.WriteAllBytesAsync(destPath, fileBytes, cancellationToken);
