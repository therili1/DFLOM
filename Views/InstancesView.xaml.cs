...
namespace Launcher.Views
{
    public sealed partial class InstancesView : Page
    {
        ...
        private async void ShowCreateDialog_Click(object sender, RoutedEventArgs e)
        {
            ...
            var cbLoaderVersion = new ComboBox { Header = "Версія завантажувача", HorizontalAlignment = HorizontalAlignment.Stretch, Visibility = Visibility.Collapsed };

            async void RebuildLoaderVersions()
            {
                var loader = cbLoader.SelectedItem?.ToString() ?? "Vanilla";
                var mcVersion = cbVersion.SelectedItem?.ToString();

                if (loader == "Vanilla" || string.IsNullOrEmpty(mcVersion))
                {
                    cbLoaderVersion.Visibility = Visibility.Collapsed;
                    cbLoaderVersion.Items.Clear();
                    return;
                }

                cbLoaderVersion.Visibility = Visibility.Visible;
                cbLoaderVersion.Items.Clear();
                cbLoaderVersion.Items.Add("Автоматично (найновіша)");
                cbLoaderVersion.IsEnabled = false;
                cbLoaderVersion.PlaceholderText = "Завантаження...";
                cbLoaderVersion.SelectedIndex = 0;

                // Захист від застарілих відповідей: якщо користувач встиг ще раз змінити
                // лоадер/версію MC поки цей запит ще летів, ігноруємо результат, що прийшов
                // не для актуального вибору - інакше список міг би "миготіти" неправильними
                // версіями лоадера від попереднього вибору.
                var requestedLoader = loader;
                var requestedMcVersion = mcVersion;

                var versions = await ViewModel.GetLoaderVersionsAsync(requestedLoader, requestedMcVersion);

                if (cbLoader.SelectedItem?.ToString() != requestedLoader || cbVersion.SelectedItem?.ToString() != requestedMcVersion)
                {
                    return;
                }

                foreach (var v in versions) cbLoaderVersion.Items.Add(v);
                cbLoaderVersion.IsEnabled = true;
                cbLoaderVersion.SelectedIndex = 0;

                if (versions.Count == 0)
                {
                    cbLoaderVersion.PlaceholderText = "Не вдалося отримати список - буде використано найновішу";
                }
            }

            cbLoader.SelectionChanged += (_, __) => RebuildLoaderVersions();
            cbVersion.SelectionChanged += (_, __) => RebuildLoaderVersions();
            ...
            stack.Children.Add(cbLoaderVersion);
            ...
            if (res == ContentDialogResult.Primary && !string.IsNullOrWhiteSpace(txtName.Text))
            {
                ...
                // Індекс 0 у cbLoaderVersion завжди "Автоматично (найновіша)" -> порожній рядок
                // (та сама семантика "auto", що вже підтримує MinecraftService.InstallInstanceAsync).
                ViewModel.NewProfileLoaderVersion = cbLoaderVersion.Visibility == Visibility.Visible && cbLoaderVersion.SelectedIndex > 0
                    ? cbLoaderVersion.SelectedItem?.ToString() ?? string.Empty
                    : string.Empty;
