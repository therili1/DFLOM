...
namespace Launcher.Services
{
    public class DownloadManager : IDownloadManager
    {
        ...
        private async Task RunDownloadAsync(DownloadTask task, string sha1Hash, string sha256Hash, CancellationToken token)
        {
            try
            {
                ...
                using (var response = await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, token))
                {
                    ...
                    {
                        // Пауза конкретної задачі: чекаємо поки Resume(id) не зніме прапорець.
                        // Status і RaiseChanged викликаємо ОДИН раз при вході і при виході з паузи,
                        // а не на кожній ітерації (кожні 200 мс) — надлишкові події флудили UI
                        // і викликали непотрібні перемальовування прогресс-бару під час паузи.
                        if (_pausedTasks.TryGetValue(task.Id, out var isPaused) && isPaused)
                        {
                            task.Status = DownloadStatus.Paused;
                            RaiseChanged(task);
                            while (_pausedTasks.TryGetValue(task.Id, out isPaused) && isPaused && !token.IsCancellationRequested)
                            {
                                await Task.Delay(200, token);
                            }
                            // Відновлення: повертаємо статус Downloading одразу після виходу з паузи.
        ...
        public void Retry(string taskId)
        {
            ...
            // Dispose the old CancellationTokenSource before replacing it to avoid a
            // resource leak — every Retry call previously created a new CTS and silently
            // orphaned the previous one in the dictionary.
            if (_cts.TryGetValue(taskId, out var oldCts))
            {
                oldCts.Dispose();
            }
            var cts = new CancellationTokenSource();
